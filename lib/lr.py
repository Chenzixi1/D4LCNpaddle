...
class _LRScheduler(object):
    def __init__(self, optimizer, last_epoch=-1):
        ...
        if last_epoch == -1:
            ...
            for i, group in enumerate(optimizer._parameter_list):
                if 'initial_lr' not in group:
                    raise KeyError("param 'initial_lr' is not specified "
                                   "in param_groups[{}] when resuming an optimizer".format(i))
        self.base_lrs = list(map(lambda group: group['initial_lr'], optimizer._parameter_list))
    ...
    def step(self, epoch=None):
        ...
        for param_group, lr in zip(self.optimizer._parameter_list, self.get_lr()):
            param_group['lr'] = lr
...
class OneCycleLR(_LRScheduler):
    ...
    Example:
        ...
        >>> optimizer = torch.optim.SGD(models.parameters(), lr=0.1, momentum=0.9)
    ...
    def __init__(self,
        ...
        if last_epoch == -1:
            for idx, group in enumerate(self.optimizer._parameter_list):
        ...
        if self.cycle_momentum:
            ...
            if last_epoch == -1:
                for m_momentum, b_momentum, group in zip(max_momentums, base_momentums, optimizer._parameter_list):
                    if self.use_beta1:
                        _, beta2 = group['betas']
    ...
    def _format_param(self, name, optimizer, param):
        ...
        if isinstance(param, (list, tuple)):
            if len(param) != len(optimizer._parameter_list):
                raise ValueError("expected {} values for {}, got {}".format(
                    len(optimizer._parameter_list), name, len(param)))
            return param
        else:
            return [param] * len(optimizer._parameter_list)
